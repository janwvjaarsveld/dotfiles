...
function M.setup()
  vim.schedule(function()
    if Util.config.news.lazyvim then
      if not Util.config.json.data.news["NEWS.md"] then
        M.welcome()
      end
      M.lazyvim(true)
    end
    if Util.config.news.neovim then
      M.neovim(true)
    end
  end)
end
...
function M.open(file, opts)
  ...
  if opts.when_changed then
    local is_new = not Util.config.json.data.news[ref]
    local hash = M.hash(file)
    if hash == Util.config.json.data.news[ref] then
      return
    end
    Util.config.json.data.news[ref] = hash
    Util.json.save()
